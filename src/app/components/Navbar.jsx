...
const Navbar = () => {
  ...
  const closeSvg = `<svg width="18" height="18" viewBox="0 0 18 18" fill="#FFF" xmlns="http://www.w3.org/2000/svg">
  ...
  return (
    <nav
      className={`${styles.paddingX} bg-primary w-full flex items-center py-5 foxed top-0 z-20`}
    >
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
        ...
        >
          <img
            src={`https://i.pinimg.com/originals/f4/1b/d5/f41bd52b3e9054798ed15822c448c18d.png`}
            className='w-9 h-9 object-contain'
          ></img>
          <p className='text-white text-[18[px] font-bold cursor-pointer'>Genshin Impact Portfolio Showcase
            <span className='sm:block hidden'>NSDAP</span>
          </p>
        </Link>
        ...
        <ul className='list-none hidden sm:flex flex-row gap-10'>
          {
            navLinks.map(data => (
              <li className={`${'active' === data.title
                ? "text-white"
                : "text-secondary"
                } hover:text-white tex-bold text-[18px]`}
